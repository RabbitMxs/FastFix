...
class classProvedor extends BaseDatos{
    var $tabla="proveedor";
    var $query="SELECT id, concat(nombre,' ',apellidoP,' ',apellidoM ) as Names, direccion as Adress from proveedor";
    ...
    function listado($query, $iconos=array() , $estilo="table-info",$ancho=array()){
        ...
        for ($r=0; $r < $this->numeTuplas; $r++) { 
            ...
            if(in_array("edit",$iconos)){
                $result.= '<td width="3%">
                    <img width="32px" src="../images/edit.svg" onclick="provedor(\'formEdit\', '.$registro['id'].')">
                </td>'; 
            }
            if(in_array("delete",$iconos)){
                $result.= '<td width="3%">
                <img width="32px" src="../images/delete.svg" onclick="provedor(\'delete\', '.$registro['id'].')">
                </td>'; 
            }
            for ($c=0; $c<$cols; $c++){
